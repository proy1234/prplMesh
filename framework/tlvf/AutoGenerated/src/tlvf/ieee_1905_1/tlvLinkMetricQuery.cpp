...
eLinkMetricNeighborType& tlvLinkMetricQueryAllNeighbors::neighbor_type() {
...
tlvLinkMetricQuerySpecificNeighbor::tlvLinkMetricQuerySpecificNeighbor(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
tlvLinkMetricQuerySpecificNeighbor::tlvLinkMetricQuerySpecificNeighbor(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
tlvLinkMetricQuerySpecificNeighbor::~tlvLinkMetricQuerySpecificNeighbor() {
}
const eTlvType& tlvLinkMetricQuerySpecificNeighbor::type() {
    return (const eTlvType&)(*m_type);
}

const uint16_t& tlvLinkMetricQuerySpecificNeighbor::length() {
    return (const uint16_t&)(*m_length);
}

eLinkMetricNeighborType& tlvLinkMetricQuerySpecificNeighbor::neighbor_type() {
    return (eLinkMetricNeighborType&)(*m_neighbor_type);
}

sMacAddr& tlvLinkMetricQuerySpecificNeighbor::mac_al_1905_device() {
    return (sMacAddr&)(*m_mac_al_1905_device);
}

eLinkMetricsType& tlvLinkMetricQuerySpecificNeighbor::link_metrics_type() {
    return (eLinkMetricsType&)(*m_link_metrics_type);
}

void tlvLinkMetricQuerySpecificNeighbor::class_swap()
{
    tlvf_swap(16, reinterpret_cast<uint8_t*>(m_length));
    m_mac_al_1905_device->struct_swap();
}

bool tlvLinkMetricQuerySpecificNeighbor::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t tlvLinkMetricQuerySpecificNeighbor::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(eTlvType); // type
    class_size += sizeof(uint16_t); // length
...
bool tlvLinkMetricQuerySpecificNeighbor::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
