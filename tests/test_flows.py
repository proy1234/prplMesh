...
class test_flows:
    ...
    def beerocks_command(self, command: str) -> bytes:
    ...
    def get_conn_map(self) -> Dict[str, map_device]:
        ...
        conn_map = {}
        for line in self.beerocks_command("bml_conn_map").split(b'\n'):
    ...
    def send_bwl_event(self, device: str, radio: str, event: str) -> None:
        ...
        command = "sh -c 'echo \"{event}\" > /tmp/$USER/beerocks/{radio}/EVENT'".format(**locals())
        self.docker_command(device, command)
