...
class LocalTestSystem(flows.TestSystem):
    ...
    def get_ip(self, device:flows.DeviceType):
    ...
    def get_port(self, device:flows.DeviceType):
        return int(get_config(device)['ucc_listener_port'])

    def get_log(self, device:flows.DeviceType, log:flows.LogType):
        log_fn = log.name + ".log"
...
if __name__ == '__main__':
    testSystem = LocalTestSystem(verbose=True)
    unittest.defaultTestLoader.loadTestsFromModule(flows)
    rv = unittest.main(testLoader=unittest.defaultTestLoader)
    testSystem.cleanup()
    sys.exit(rv)
